...
* Не переусердствуйте, но и не давайте программе аллоцировать/деаллоцировать очень много (тысячи в секунду)
...
* Для простоты можно считать, что их можно получить с помощью адресации стека или аллокации heap
...
* `nullptr` работает со всеми случаями, `NULL` это C константа
...
* Чтобы передавать управление другим сущностями
...
* Всё больше стайлгайдов говорят, что указатели никогда ничего не хранят (то есть они не из кучи или кто-то другой за них ответственный). Подразумевайте это по умолчанию, иначе ошибётесь и запутайтесь
...
```cpp
// Why do we need strings? string_view is enough.
std::vector<std::string_view> v = std::views::split("a,b,c", ',');
```
...
* Передавать по значению (на семинарах расскажут почему)
...
* В произвольном порядке создаются и разрушаются
...
* Все строки в `""` -- глобальные переменные
