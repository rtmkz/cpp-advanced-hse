...
Можно использовать Windows, но для компиляции и запуска нужно будет использовать WSL, подробная инструкция [здесь](docs/windows.md).
...
## Сдача первой задачи
 ...
 - Запустите скрипт посылки из директории с задачей: 
   * Linux & OS X: `python3 ../../submit.py`
 
   * Windows: `python ..\..\submit.py`, если `python` не определяется, то убедитесь, что он установлен и путь к `python.exe` прописан в PATH
...
**Важно!** Тем, кто использует связку CLion + WSL, выполнять эти команды нужно из Windows PowerShell. Лучше всего для этого открыть вкладку Terminal в CLion (снизу), так вы сразу окажетесь в каталоге с репозиторием.
