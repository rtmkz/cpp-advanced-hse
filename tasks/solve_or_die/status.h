...
// Example:
//
// absl::Status myFunction(std::string_view fname, ...) {
//   ...
//   // encounter error
//   if (error condition) {
...
public:
    ...
    // `std::string` to hold the contextual data.
    //
    ...
    // passing it the type URL and an `std::string` of associated data. Similarly,
...
private:
    ...
    static uintptr_t NewRep(absl::StatusCode code, std::string_view msg,
...
inline std::string_view Status::message() const {
    return !IsInlined(rep_)
               ...
               : (IsMovedFrom(rep_) ? std::string_view(kMovedFromString) : std::string_view());
}
