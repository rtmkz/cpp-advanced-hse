...
- В этом задании вам разрешено менять `CMakeLists.txt`, в значит вы можете добавлять свои файлы с исходниками, а также подкаталоги. Напомним, что если вы добавляете `.cpp`, нужно указать его в списке исходников библиотеки в [CMakeLists.txt](./CMakeLists.txt).
