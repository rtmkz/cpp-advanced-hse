...
Также имплементируйте метод `Move`, реализующий интерфейс движения по ребру с заданным значением `bit` дерева из текущей вершины. Если конечная вершина ребра является листом, значение в листе нужно записать в переданную переменную и вернуть `true`. Значение в текущей вершине при этом сбрасывается.
...
В качестве стартового корпуса будем использовать изображения из [tests](./tests). Путь к каталогу с корпусом передавайте в аргументах командной строки:

```
./fuzz_jpeg path_to_tests_directory
```
