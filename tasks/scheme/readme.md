...
| 5   | **[tidy](tidy/README.md)**           | 1 балла   | При решении подзадачи `advanced` вы наверняка столкнетесь с утечками памяти - и в `advanced` мы не проверяем, очищена ли память. Чтобы избавиться от утечек, нужно написать свой сборщик мусора - это вы и сделаете в `tidy`. |
