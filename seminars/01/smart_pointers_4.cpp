...
void UPAccess1(const std::unique_ptr<int>& ptr) {
  puts("Kind of weird, we don't really need to know that the object is stored in a smart pointer.");
  *ptr = 21;
  printf("Value is %d.\n", *ptr);
}
...
int main() {
  ...
  // UPTakeOwnership1(up);  // doesn't compile, which is exactly what we want if ownership is passed.
