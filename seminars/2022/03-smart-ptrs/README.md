...
Сначала стоит обсудить, как правильно и как неправильно копировать `SharedPtr`. А что делать, если в какой-то точке программы у вас нет `SharedPtr<T>`, есть только `T*`, и нужно получить корректный `SharedPtr<T>`? Можем ли мы научить сам объект типа `T` увеличивать счетчик ссылок на себя?
...
* Зачем нужна такая сложная схема с нешаблонным `ESFTBase` и проверкой через `std::is_convertible_v<T*, ESFTBase*>`? Почему не `std::is_base_of_v<T, ESFT<T>>`?

* Что делать, если тип `T` имеет свое поле под названием `weak_this`?
